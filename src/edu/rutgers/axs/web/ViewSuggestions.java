...
public class ViewSuggestions extends PersonalResultsBase {
	...
	String s = "";
	if (onTheFly) {
	    s += "<p>Initial suggestion list generated in runtime.</p>\n";
	} else {
	    s += "<p>Suggestion list " + df.getThisFile() + 
		" was generated for user " + df.getUser() + " at: " + 
		Util.ago(df.getTime()) + ".\n"; 

	    String q = "The list was generated by applying the user profile " +
"<a href=\"" + viewProfileLink(df.getInputFile()) + "\">" + 
		df.getInputFile() + "</a> to the " + 
		(days>0? "articles from the last " + days + " days" :
